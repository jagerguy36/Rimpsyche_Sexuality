...
namespace Maux36.RimPsyche.Sexuality
{
    ...
    public static class Pawn_SexualityTracker_Initialize
    {
        static bool Prefix(Pawn_SexualityTracker __instance, Pawn pawn, bool rewrite)
        {
            ...
            if (__instance.orientationCategory != SexualOrientation.None && __instance.orientationCategory != SexualOrientation.Developing)
            {
                return false;
            }
            var traits = pawn.story?.traits;
            var gender = pawn.gender;
            if (traits != null && gender != Gender.None)
            {
                ...
                {
                    __instance.orientationCategory = SexualOrientation.Homosexual;
                }
                else if (traits.HasTrait(TraitDefOf.Bisexual))
                {
                    __instance.orientationCategory = SexualOrientation.Bisexual;
                }
                else if (traits.HasTrait(TraitDefOf.Asexual))
                {
                    __instance.orientationCategory = SexualOrientation.Asexual;
                }
                else
                {
                    __instance.orientationCategory = SexualOrientation.Heterosexual;
                }
